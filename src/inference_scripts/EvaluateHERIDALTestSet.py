...
train_data_path = args.in_folder_path
...
ds = HERIDALDatasetAdaptor(train_data_path, labels_df)
...
with alive_bar(len(ds), bar="classic", enrich_print=False) as prog_bar:
	for idx in range(0, len(ds)):
		...
		bounding_boxes, error_code = tiled_image_inference(model, image, tile_dim, batch_size=args.batch_size)
...
preds_filename = "preds-" + model_name + "-" + str(tile_dim) +  "-" + str(model.prediction_confidence_threshold) + ".csv"
